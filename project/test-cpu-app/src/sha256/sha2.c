...
void perform_compression(word_t *msg_schedule, word_t *hash) {
    ...
    for (int i = 0; i < MSG_SCHEDULE_SIZE_WORDS; i++) {
        ...
        #ifdef DEBUG
            xil_printf("t1: %08X\nt2: %08X\n", temp1, temp2);
            xil_printf("msg[i]: %08X\nk[i]: %08X\n", msg_schedule[i], k[i]);
            xil_printf("i=%d\t%08X\t%08X\t%08X\t%08X\t%08X\t%08X\t%08X\t%08X\n\n",
                i, a, b, c, d, e, f, g, h 
            );
        #endif
    }
...
bool add_padding(byte_t* msg_block, uint64_t padding_start, uint64_t msg_size) {
    ...
    if ((padding_start * 8) >= (NUM_MSG_BLOCK_BITS - MIN_PAD_LENGTH_BITS - 1)) {
        ...
        #ifdef DEBUG
            xil_printf("starting overlapping padding\n");
        #endif
    }
    ...
    // add remainder of the padding to the last block
        ...
        #ifdef DEBUG
            xil_printf("finishing overlapping padding\n");
        #endif
    }
    ...
    // the entire padding fits on one block
        ...
        #ifdef DEBUG
            xil_printf("adding entire padding to block\n");
        #endif
    }
    ...
    if (finish_padding) {
        ...
        #ifdef DEBUG
            xil_printf("fin padding, msg size: %08lX: \n", msg_size * 8);
        #endif
...
void output_hash(byte_t* hash) {
    xil_printf("sha256 hash:\n\r");
    for (int i = 0; i < SHA256_SIZE_BYTES; i++) {
        xil_printf("%02X", hash[i]);
    }
...
//         switch (arg_type) {
//             case ARGT_FILE:
//                 xil_printf("Error: no file path.\n");
//                 break;
            
//             case ARGT_STRING:
//                 xil_printf("Error: no string.\n");
//                 break;

//             case ARGT_INVALID: break;
//             default:
//                 xil_printf("Invalid arg(s). Run without any args to see possible args.\n");
//         }
//     }  
//     else if (argc >= 3) {
//         switch (arg_type) {
